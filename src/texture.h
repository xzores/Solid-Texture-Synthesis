...
int stocastic_solid_texture_synthesis(const char* texture_file)
{
    ...
    for(int i=0; i<rows; i+=50){
        for(int j=0; j<cols; j+=50){
            ...
            neighbour_x = rand() % (original_texture.size().width); //Select a random neighbourhood
            neighbour_y = rand() % (original_texture.size().height);
            neighbour_z = rand() % (original_texture.size().width);
            
            int count = 1;
            while(err > 9.8){
                random_x = neighbour_x - 75 + rand() % 150; //Select a patch from the neighbourhood
                random_y = neighbour_y - 75 + rand() % 150;
                random_z = neighbour_z - 75 + rand() % 150;
                if((random_x <= 25 or (random_x >= (original_texture.size().width - 25)))
                    or (random_y <= 25 or (random_y >= (original_texture.size().height - 25)))
                    or (random_z <= 25 or (random_z >= (original_texture.size().width - 25)))) continue;
                else{
                    patch = original_texture(Range(random_y-25,random_y+25), 
                                            Range(random_x-25,random_x+25), 
                                            Range(random_z-25,randomz+25));
                    if(flag) err = compare(prev_patch, patch);
                    else err = 0;
                }
                if(count++ % 10){
